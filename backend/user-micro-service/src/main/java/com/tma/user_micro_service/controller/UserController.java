...
<<<<<<< Updated upstream
	...
	public ResponseEntity<StandardResponse<User>> updateUser(@PathVariable UUID userId, @RequestBody User user, HttpServletRequest request) {
		try {
			...
			return ResponseUtil.buildSuccessMessage(HttpStatus.OK, "User updated successfully", updatedUser, request, LocalDateTime.now());
			
		} catch (Exception e) {
			
			return ResponseUtil.buildErrorMessage(HttpStatus.INTERNAL_SERVER_ERROR, "An error occurred while updating the user", request, LocalDateTime.now());
		}
	}
	...
	public ResponseEntity<StandardResponse<User>> deleteUser(@PathVariable UUID userId, HttpServletRequest request) {
		try {
			...
			return ResponseUtil.buildSuccessMessage(HttpStatus.NO_CONTENT, "User deleted successfully", null, request, LocalDateTime.now());
			
		} catch (Exception e) {
			
			return ResponseUtil.buildErrorMessage(HttpStatus.INTERNAL_SERVER_ERROR, "An error occurred while deleting the user", request, LocalDateTime.now());
		}
	}
