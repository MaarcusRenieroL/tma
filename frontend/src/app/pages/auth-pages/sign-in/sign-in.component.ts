      ...
      if (response) {
        if (response.data.token) {
          ...
          this.cookieService.set(
            'syncTeam.isOnboarded',
            response.data.onboarded.toString()
          );
          this.cookieService.set(
            'syncTeam.isVerified',
          ...
          // Redirect based on onboarding and verification status
          if (response.data.verified && response.data.onboarded) {
            this.router.navigate(['/dashboard']).then();
          } else if (response.data.verified && !response.data.onboarded) {
