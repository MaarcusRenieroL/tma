...
@Component({
  selector: "developer-notifications",
  templateUrl: "./developer-notifications.component.html",
  styleUrl: "./developer-notifications.component.css",
})
export class DeveloperNotificationsComponent {
  ...
  generateNotifications(): Notification[] {
    ...
    const types: ("alert" | "update" | "chat" | "task" | "meeting")[] = [
      "alert",
      "update",
      "chat",
      "task",
    ...
    for (let i = 1; i <= 20; i++) {
      ...
      const timestamp = new Date(
        baseTimestamp.getTime() + i * 1000 * 60 * 60,
      ).toISOString();
      ...
      if (type === "chat") {
        ...
        notification.meta = {
          ...
          deadline: new Date(
            baseTimestamp.getTime() + (i + 2) * 1000 * 60 * 60,
          ).toISOString(),
        };
      } else if (type === "meeting") {
        notification.meta = {
          meetingTime: new Date(
            baseTimestamp.getTime() + (i + 1) * 1000 * 60 * 60,
          ).toISOString(),
  ...
  filterNotifications(tab: string): void {
    ...
    if (tab === "all") {
      ...
      this.filteredNotifications = this.notifications.filter(
        (notification) => notification.type === tab,
      );
    }
  }
